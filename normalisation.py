...
def process_audio(file_path):
    ...
    #std = 2
    #bin_std = 2.5  
    #bin_per = 0.2 # Retains only the top 20% of pixel intensities
    #nperseg = 8192  
    #noverlap = int(0.8 * nperseg)  
    std = 2
    bin_std = 3 
    bin_per = 0.25 # Retains only the top 20% of pixel intensities
    nperseg = 8192  
    noverlap = 4096
    ...
    s = (s - s.mean()) / s.std()
    print(f'Normalized audio standard dev: {s.var()}')
    ...
    if not df_rois.empty:
        ...
        low_freq_rois = df_rois[df_rois['min_f'] >= 50]
        low_freq_rois = low_freq_rois[(low_freq_rois['max_t'] - low_freq_rois['min_t']) >= 0.03]
        low_freq_rois = low_freq_rois[(low_freq_rois['max_t'] - low_freq_rois['min_t']) < 10]
    else:
        low_freq_rois = pd.DataFrame()

    # Plot the spectrogram with ROIs
    fig, ax = plt.subplots(figsize=(10, 4))
    img = ax.imshow(Sxx_db, aspect='auto', origin='lower', extent=ext, vmin=0, vmax=60, cmap='magma', interpolation='bilinear')
